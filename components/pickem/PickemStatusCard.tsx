...
const VISUAL_CONFIG: Record<VisualState, {
  dot: string;
  title: string;
  description: string;
  nextStep: string;
  ...
  completed: {
    ...
    description: 'Resultados finales publicados y clasificación disponible.',
    nextStep: 'Revisar la clasificación final.',
  },
};
...
export function PickemStatusCard({
  ...
  const visualState: VisualState =
    status === 'completed' && pendingTiebreakerCount > 0
      ? 'tiebreakers_pending'
      : status === 'completed'
        ? 'completed'
        : status;

  const config = VISUAL_CONFIG[visualState];
    ...
    <>
      <div className="rounded-xl border border-border bg-surface p-5 sm:p-6">
        ...
        <div className="flex items-start gap-3">
          <span className={`mt-1 flex size-3 shrink-0 rounded-full ${config.dot}`} />
          <div className="flex-1 min-w-0">
            ...
            {visualState === 'open' && closeDate && (
              <p className="mt-2 text-xs text-text-muted">
                Cierre automático: {formatDate(closeDate)}
              </p>
            )}

            {visualState === 'predictions_closed' && (
              <p className="mt-2 text-xs text-text-muted">
                {submissionCount} participación{submissionCount !== 1 ? 'es' : ''} recibida{submissionCount !== 1 ? 's' : ''}
              </p>
            )}
            ...
            {visualState === 'completed' && (
              <p className="mt-2 text-xs text-text-muted">
                {submissionCount} participación{submissionCount !== 1 ? 'es' : ''} · Clasificación final disponible
              </p>
            )}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-medium text-text-muted">PRÓXIMO PASO</p>
            <p className="mt-0.5 text-sm text-text-primary">{config.nextStep}</p>
          </div>
          ...
          <div className="shrink-0">
            ...
            {visualState === 'completed' && (
              <a
                ...
                className="inline-flex w-full sm:w-auto items-center justify-center rounded-lg bg-success px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-success/80"
              >
                Ver resultados
              </a>
            )}
      ...
      {/* Close predictions confirmation modal */}
      {showCloseModal && (
        <ConfirmActionModal
